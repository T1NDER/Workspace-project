...
export function ActiveProjectsRotator() {
  const { visibleItems, isPaused, setIsPaused, startIndex } = useRotatingItems(projects, 2, 4000);
    ...
    <section className="rotator">
      <div className="rotator__container">
        <div className="rotator__header">
          ...
          <button onClick={() => setIsPaused(!isPaused)} className="rotator__toggle-btn">
            ...
            <article key={project.id} className="rotator__card fade-in">
              ...
              <div className="rotator__progress">
                ...
                <div className="rotator__progress-track">
                  <div className="rotator__progress-fill" style={{ width: `${project.progress}%` }}></div>
                </div>
              </div>
        ...
        <div className="rotator__dots">
          {Array.from({ length: totalPairs }).map((_, i) => (
            <div key={i} className={`rotator__dot ${i === currentPair - 1 ? "rotator__dot--active" : ""}`}></div>
          ))}
        </div>
      </div>
    </section>
